...
const Edit = ({ selectedCard, setIsEditing }) => {
  ...
  const [tags, setTags] = useState((selectedCard.tags && selectedCard.tags.join(', ')) || '');
  ...
  const [position, setPosition] = useState(selectedCard.position || 'left'); // New state for position
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const docRef = doc(db, 'other-services', id);
