...
const Table = () => {
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await deleteDoc(doc(db, 'other-services', id));
  ...
  return (
    <div>
      <SpaceBetween>
        <StyledButton onClick={handleAddClick}>Add Service</StyledButton>
      </SpaceBetween>
      <h1>
      ...
      />
      <StyledButton onClick={handleEditDeposit}>Edit Service Title</StyledButton>
      {!isEditing && !isAdding && (
        <>
          <StyledTable>
            <thead>
