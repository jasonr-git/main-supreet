...
const DescriptionUpdater = () => {
  ...
  useEffect(() => {
    ...
    async function fetchDescription() {
      try {
        const docRef = doc(db, 'origin', 'desc'); // Update 'desc' with the actual ID
  ...
  return (
    <Container>
      <h1>Origin English</h1>
      <StyledInput
        placeholder="Enter new description"
        value={description}
        onChange={handleDescriptionChange}
      />
      <StyledButton onClick={handleUpdateDescription}>Update Description</StyledButton>
    </Container>
  );
};
